...
def parse_args():
    ...
    parser.add_argument(
        '--intermediate-dir',
        default=None,
        help='directory to feed KV260 outputs for visualization.'
        'See postprocess() for what files are expected.')
    parser.add_argument(
        '--gpu-collect',
        action='store_true',
...
def postprocess(float_model: nn.Module, intermediate_dir: Path):
    # Check the directory contents.
    npys = list(intermediate_dir.glob('*.npy'))
    data_path = intermediate_dir / 'data.pkl'
    vis_dir = intermediate_dir / 'visualized'
    if len(npys) != 3:
        msg = 'Set a directory from pointpillars.py --output-dir' \
              '(e.g. --intermediate-dir=./2023-01-16_04-20-50/)'
        raise FileNotFoundError(msg)
    if not data_path.exists():
        msg = 'Pickle `data` in `single_gpu_quant_test()` below for visualization.'
        raise FileNotFoundError(msg)
    if vis_dir.exists():
        msg = "Move the past visualization results to somewhere else."
        raise FileExistsError(msg)

    # Pack KV260 outputs as a list.
    outs = []
    for npy in npys:
        t = torch.from_numpy(np.load(npy))

        # Vitis AI permutes from BCHW to BHWC during compilation,
        # thus re-permute BHWC to BCHW here.
        t = torch.permute(t, (0, 3, 1, 2))

        # `float_model.postprocess_api()` only supports cuda.
        outs.append(t.to(torch.device('cuda')))

    # "Anchor3DHead_quant" outputs 3 tensors in descending order of C in BCHW.
    outs = sorted(outs, key=lambda t: t.shape[1], reverse=True)

    # Feed `outs` into `float_model.postprocess_api()` then visualize the result.
    with open(data_path, 'rb') as f:
        data = pickle.load(f)
        img_metas = data['img_metas'][0].data[0]
    result = float_model.postprocess_api(outs, img_metas, rescale=True)
    vis_dir.mkdir()
    float_model.show_results(data, result, vis_dir)
    ...
    if args.fast_finetune:
        if quant_mode == 'calib':
            ...
            if args.intermediate_dir:
                postprocess(float_model, Path(args.intermediate_dir))
                sys.exit()
